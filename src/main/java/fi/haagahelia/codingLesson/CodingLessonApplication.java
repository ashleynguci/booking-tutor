...
public class CodingLessonApplication {
	...
	return (args) -> {
		...
		langRepository.save( new LanguageEntity("Spring Boot"));
			...
			tutorRepository.save( new TutorEntity("Katie Nguyen"));
			tutorRepository.save( new TutorEntity("Vesa Huotarinen"));
			tutorRepository.save( new TutorEntity("Felix Bloch"));
			tutorRepository.save( new TutorEntity("Carlos Sunila"));
			tutorRepository.save( new TutorEntity("Markus Leppa"));

		lessonRepository.save( new LessonEntity("2019-06-20 15:00","2019-06-20 18:00", "Arentikuja 1D304", 4, langRepository.findByName("Javascript").get(0), tutorRepository.findByName("Vesa Huotarinen").get(0)));
		lessonRepository.save( new LessonEntity("2019-06-15 09:00","2019-06-15 13:00", "Hamenkatu 10A", 5, langRepository.findByName("C#").get(0), tutorRepository.findByName("Katie Nguyen").get(0)));
		lessonRepository.save( new LessonEntity("2019-06-21 19:00","2019-06-21 21:30", "Aurakatu 123", 3, langRepository.findByName("React").get(0), tutorRepository.findByName("Carlos Sunila").get(0)));
		lessonRepository.save( new LessonEntity("2019-06-20 14:00","2019-06-20 16:00", "Yo kyla  5D7", 4, langRepository.findByName("Javascript").get(0), tutorRepository.findByName("Carlos Sunila").get(0)));
