...
public class CodingLessonApplication {
	...
	public CommandLineRunner demo(LessonRepository lessonRepository, LanguageRepository langRepository,
		...
		return (args) -> {
			...
			lessonRepository.save(new LessonEntity(format.parse("2019-06-16"), "15:00", "18:00", "Arentikuja 1D304", 4,
					langRepository.findByName("Javascript").get(0),
					tutorRepository.findByName("Vesa Huotarinen").get(0), true));
			lessonRepository.save(new LessonEntity(format.parse("2019-06-12"), "09:00", "13:00", "Hamenkatu 10A", 5,
					langRepository.findByName("C#").get(0), tutorRepository.findByName("Katie Nguyen").get(0), true));
			lessonRepository.save(new LessonEntity(format.parse("2019-06-18"), "19:00", "21:30", "Aurakatu 123", 3,
					langRepository.findByName("React").get(0), tutorRepository.findByName("Carlos Sunila").get(0),
					true));
			lessonRepository.save(new LessonEntity(format.parse("2019-06-22"), "14:00", "16:00", "Yo kyla  5D7", 4,
					langRepository.findByName("Javascript").get(0), tutorRepository.findByName("Carlos Sunila").get(0),
					true));
			...
			userRepository
					.save(new User("admin", "$2a$10$OPO.1kKxqwU7m8aCkBlm5OYWGbRx2ol9k6N382WJP6UsjFsGlJZq2", "ADMIN"));
